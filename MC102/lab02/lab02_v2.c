...
int main (int argc, char *argv[])
{
	...
	int numilhas; //numero de ilhas
	double ilhax1, ilhay1, ilhax2, ilhay2; //coordenadas serão comparadas duas a duas
	double xprimeiro, yprimeiro; //primeiras coordenadas da ilha.
	double xmax, ymax, xmin, ymin; //coordenadas para definir as bordas da maquete
	double area, areasoma; //area auxiliar para calcular a superficie das ilhas e a area somatória
	short int aux; //variavel auxiliar para realização de contagem
	scanf("%d", &numilhas);
	for (int i = 0; i < numilhas; i++)
	{
		...
		//tarefa de atribuição de valores as bordas da maquete
		if (xmax < ilhax1)
			xmax = ilhax1;
		if (xmin > ilhax1)
			xmin = ilhax1;
